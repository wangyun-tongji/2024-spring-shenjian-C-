	...
	for (int i = 1; i <= N_NUM; i++)
	{
		if (int(d_cur[i]) != 0)
		{
			if ((int(d_cur[i]) - 48)% 2 == 0)
			{
				if (int(d_next[i]) == 0)
				{
					d_next[i] = char(48 + (int(d_cur[i]) - 48)/ 2);
				}
				else
				{
					d_next[i] = char(48+ (int(d_cur[i]) - 48)/ 2 + int(d_next[i]) - 48);
				}
			}
			else
			{
				...
				if (d_next[i + 1] == 0)
				{
					d_next[i + 1] = char(5 +48 + int(d_next[i + 1]));
			...
			for (int j = N_NUM; j > 0; j--)
			{
				if (power_table[i][j] != 0)
				{
					if (int(out[j]) == 0)
					{
						...
						if (int(out[j]) - 48 >= 10)
						{
							out[j] = char((int(out[j])-48)%10+48);
							out[j - 1] = char(int(out[j - 1]) + (int(out[j]) - 48)/10);
						}
						else
							;
					}
...
int main()
{
	...
	*/
	...
	power_table[1][1] = char(5+'0'); // 2^-1 = 0.5，先置[1][1]为5
